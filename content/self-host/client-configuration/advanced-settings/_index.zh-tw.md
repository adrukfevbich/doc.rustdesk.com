...
這不同於 [`privacy-mode`](#privacy-mode)：後者會為每個對等端設定首次連接後的預設隱私模式行為，而 `enable-privacy-mode` 控制是否允許使用隱私模式。

**位置**：

1. **桌面** 設定 → 安全 → 權限 → 啟用隱私模式

| 安裝需要 | 值 | 預設值 | 範例 |
| :------: | :------: | :------: | :------: |
| 否 | Y, N | Y | `enable-privacy-mode=Y` |
...
在傳入的遠端桌面工作階段期間保持螢幕常亮。這有助於防止裝置在主動使用應用程式進行遠端連線時進入休眠。
...
在傳出的遠端桌面工作階段期間保持螢幕常亮。這有助於防止裝置在主動使用應用程式進行遠端連線時進入休眠。
...
控制是否允許控制端點擊接受視窗（連線管理視窗）來接受連線、更改權限等。
...
控制使用者在接受傳入會話前，是否可以在接受視窗（連線管理視窗）中更改權限。

在桌面端，此選項會影響接受視窗（連線管理視窗）中的所有權限。在 Android 上，此選項只影響以下權限：傳輸檔案、音訊錄製和啟用剪貼簿；不影響畫面錄製和輸入控制。

**預覽**： https://github.com/rustdesk/rustdesk/pull/14875
...
如果你在客戶端介面中啟用此選項但尚未登入，RustDesk 會先要求你登入。

**位置**:

1. **桌面** 設定 → 其他 → 在連接結束時請求備註
2. **行動裝置** 設定 → 在連接結束時請求備註

自 RustDesk 1.4.4 起可用。

**討論**: https://github.com/rustdesk/rustdesk/discussions/14606
...
禁止更改永久密碼。啟用後，使用者無法透過介面或命令列設定或修改永久密碼。

自 RustDesk 1.4.5 起可用
...
禁止更改裝置 ID。啟用後，使用者無法透過介面或命令列更改 ID。
...
禁止使用 PIN 解鎖設定。啟用後，即使已設定 PIN，使用者也必須使用系統管理員權限來解鎖設定。
