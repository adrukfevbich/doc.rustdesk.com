...
1. **デスクトップ** 設定 → セキュリティ → 権限 → クリップボードを有効化する
2. **モバイル**
...
1. **デスクトップ** 設定 → セキュリティ → 権限 → ファイル転送を有効化する
2. **モバイル**
...
1. **デスクトップ** 設定 → セキュリティ → 権限 → オーディオを有効化する
2. **モバイル**
...
被制御側が制御側にプライバシーモードの使用を許可するかどうかを制御します。

これは [`privacy-mode`](#privacy-mode) とは異なります。この設定は最初の接続後に各ピアのデフォルトのプライバシーモード動作を設定しますが、`enable-privacy-mode` はプライバシーモードを許可するかどうかを制御します。

**場所**：

1. **デスクトップ** 設定 → セキュリティ → 権限 → プライバシーモードを有効化する
...
受信するリモートデスクトップセッション中に画面をスリープさせず、点灯したままにします。これにより、リモート接続のためにアプリをアクティブに使用している間、デバイスがスリープするのを防げます。
...
送信するリモートデスクトップセッション中に画面をスリープさせず、点灯したままにします。これにより、リモート接続のためにアプリをアクティブに使用している間、デバイスがスリープするのを防げます。
...
制御側が承認ウィンドウ（接続管理画面）をクリックして接続を受け入れたり、権限を変更したりできるかどうかを制御します。
...
受信セッションを承認する前に、ユーザーが承認ウィンドウ（接続管理画面）で権限を変更できるかどうかを制御します。

デスクトップでは、承認ウィンドウ（接続管理画面）内のすべての権限に影響します。Android では、次の権限に影響します：ファイルを転送、音声キャプチャ、クリップボードを有効化する。画面キャプチャと入力操作には影響しません。

**プレビュー**: https://github.com/rustdesk/rustdesk/pull/14875
...
サービスの実行中は、サービスの停止/切り替え操作を非表示にします。主にカスタムクライアント向けで、ユーザーが UI（デスクトップ設定、トレイメニュー、Android サーバーページ、Android フローティングウィンドウメニュー）からサービスを停止するのを防ぐためのものです。

サービスが停止している場合、開始/有効化の項目は表示されたままになります。
...
接続終了時に、制御側にメモの入力を求めます。

クライアントUIからこのオプションを有効にし、まだログインしていない場合、RustDesk は先にログインを求めます。

**場所**:

1. **デスクトップ** 設定 → その他 → 接続終了時にメモを要求する
2. **モバイル** 設定 → 接続終了時にメモを要求する

RustDesk 1.4.4 以降で利用可能。

**ディスカッション**：https://github.com/rustdesk/rustdesk/discussions/14606
...
永続パスワードの変更を無効にします。有効にすると、ユーザーはUIまたはコマンドラインから永続パスワードを設定または変更できなくなります。

RustDesk 1.4.5 以降で利用可能
...
デバイスIDの変更を無効にします。有効にすると、ユーザーはUIまたはコマンドラインからIDを変更できなくなります。
...
PINによる設定のロック解除を無効にします。有効にすると、PINが設定されていても、ユーザーはシステム管理者権限を使用して設定をロック解除する必要があります。
