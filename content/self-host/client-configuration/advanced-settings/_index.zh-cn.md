...
这不同于 [`privacy-mode`](#privacy-mode)：后者会为每个对等端设置首次连接后的默认隐私模式行为，而 `enable-privacy-mode` 控制是否允许使用隐私模式。

**位置**：

1. **桌面** 设置 → 安全 → 权限 → 允许隐私模式
...
在传入的远程桌面会话期间保持屏幕常亮。这有助于防止设备在主动使用应用进行远程连接时进入休眠。
...
在传出的远程桌面会话期间保持屏幕常亮。这有助于防止设备在主动使用应用进行远程连接时进入休眠。
...
控制是否允许控制端点击接受窗口（连接管理窗口）来接受连接、更改权限等。
...
控制用户在接受传入会话前，是否可以在接受窗口（连接管理窗口）中更改权限。

在桌面端，此选项会影响接受窗口（连接管理窗口）中的所有权限。在 Android 上，此选项只影响以下权限：传输文件、音频录制和允许同步剪贴板；不影响屏幕录制和输入控制。

**预览**： https://github.com/rustdesk/rustdesk/pull/14875
...
在服务运行时隐藏停止/切换服务的控件。这主要用于定制客户端，防止用户从界面中停止服务（桌面设置、托盘菜单、Android 服务器页面和 Android 悬浮窗菜单）。
